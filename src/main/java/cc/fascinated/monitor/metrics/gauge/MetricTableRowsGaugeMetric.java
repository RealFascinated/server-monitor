...
@Component
public class MetricTableRowsGaugeMetric implements RefreshableMetric {
    private final GaugeHolder gauge;
    private final PlatformMetricsQueryService queries;

    public MetricTableRowsGaugeMetric(PrometheusRegistry registry, PlatformMetricsQueryService queries) {
        this.gauge = GaugeHolder.register(registry, "monitor_table_rows", "Row count per metrics table", "table");
    ...
    public void refresh() {
        for (String table : this.queries.metricTables()) {
            this.gauge.set(this.queries.tableRowCount(table), table);
        }
    }
}
